...
class struct2df():
    ...
    def __init__(self, matfile, separate_chambers=None):
        ...
        if 'trx' in self.mat_dict.keys():
            ...
            #####Checking for lost tracking identities##########
            data_lengths = []
            for trx in self.trx_ls:
                data_lengths.append(len(trx))
            fault = False
            length_of_video = max(data_lengths)
            lost_flies = []
            for idx, length in enumerate(data_lengths):
                if length != length_of_video:
                    lost_flies.append(str(idx+1))
                    fault = True
                else:
                    continue
            if fault:
                raise self.FaultyTrackingError(lost_flies)
            ####################################################
            ...
            for k, v in scores_dict.items():
                if len(v) != length_of_video:
                    lost_flies.append(str(k))
                    fault = True
                else:
                    continue
            if fault:
            ...
            #####Checking for lost tracking identities##########
            data_lengths = []
            for v in new_perframe.values():
                data_lengths.append(len(v))
            fault = False
            length_of_video = max(data_lengths)
            lost_flies = []
            for k, v in new_perframe.items():
                if len(v) != length_of_video:
                    lost_flies.append(str(k))
                    fault = True
                else:
    ...
    #inner exception class for faulty tracking data
    class FaultyTrackingError(Exception):
        """
        Exception raised when imported data is indicative of failed tracking and lost fly identities/major identity swaps.
        """
        def __init__(self, lost_flies):
            self.message = "\n\nTracking failed for this video! Fly # " + ", ".join(lost_flies) + " lost during tracking.\n"
    ...
    def plot_tracks(self, bysex=False, burnin=0, plottitle='', saveplot=True, filename='', showplot=False):
        ...
        if self.dtype == "trx":
            ...
            #does x and y data need to me extracted?
            if 'x_mm_1' and 'y_mm_1' not in self.param_df.columns:
                self.extract_trx_param(['x_mm', 'y_mm'], savefile=False)
                print("\nWARNING: `plot_tracks` has used the `extract_trx_param` method to extract x_mm and y_mm data and has removed any previously extracted data from the `.param_df` attribute. You will have to rerun `extract_trx_param` to get those data back in the `.param_df`.\n")
            ...
            if self.chambers == None:
                ...
                if bysex == True:
                    ...
                    for idx, i in enumerate(self.trx_ls):
                        ...
                        x = self.param_df['x_mm_{id}'.format(id=str(int(idx+1)))].to_list()[burnin:]
                        y = self.param_df['y_mm_{id}'.format(id=str(int(idx+1)))].to_list()[burnin:]
                ...
                if saveplot:
                    plt.savefig('{name}x_y_tracks.png'.format(name=filename))
                ...
                if bysex == True:
                    ...
                    for cham in list(self.chambers.keys()):
                        ...
                        for indv in self.chambers[cham]:
                            ...
                            x = self.param_df['x_mm_{id}'.format(id=str(int(indv)))].to_list()[burnin:]
                            y = self.param_df['y_mm_{id}'.format(id=str(int(indv)))].to_list()[burnin:]
                        ...
                        if saveplot:
                            plt.savefig('{name}_Chamber_{ch}_x_y_tracks.png'.format(name=filename, ch=cham))
